import React, { FC, useState, ReactElement, useEffect, KeyboardEvent, useRef, ChangeEvent } from 'react';
...
const renderTemplate=(item:DataSourceType,renderOption:((item:DataSourceType)=>ReactElement)|undefined)=>{
    return renderOption?renderOption(item):item.value
}

const generateDropDown=(dropdown:DataSourceType[],selectCallback:((item:DataSourceType)=>void)|undefined,
    ...
    useEffect(()=>{
        if(debouncedValue&&triggerSearch.current){
            ...
            if(res instanceof Promise){
                setLoading(true)
                res.then((v)=>{setDropDown(v);setLoading(false)})
            }else{
                setDropDown(res)
