...
export const List:FC<ListProps> = (props)=>{
    const {className,mode,style,children,onSelect,renderTemplate,listyle,liClassName,withHoverActive,refCallback}=props 
        ...
        <ul className={classes} style={style} onClick={(e)=>handleClick(e)} ref={ulref} >
            {
                ...
                const res =renderTemplate? <li key={index} style={listyle} className={liclasses}>{renderTemplate(child,index)}</li> :(
                    <li key={index}  className={liclasses} style={listyle}>{child}</li>
                    )
                    return res
                })
