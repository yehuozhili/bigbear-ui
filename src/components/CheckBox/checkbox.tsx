...
function CheckBox(props:PropsWithChildren<CheckBoxProps>){
    const {defaultIndexArr,callback,data,className,disableIndex,parentSetStateCallback,parentState}=props
        ...
        <div className={classes}>
            {
                data.map((value,index)=>{
                    ...
                    return(
                        <label className={`bigbear-checkbox-label ${disableRef[index]?'checkbox-disabled':''}`} key={index}>
                            <input type='checkbox' className='bigbear-checkbox-input' 
                            checked={judgeStateIndex?true:false} 
                            onChange={()=>{  
                                if(parentState){
                                    if(parentSetStateCallback)parentSetStateCallback(parentState,index)
                            ...
                            <span className={`bigbear-checkbox-dot ${judgeStateIndex?'checkbox-active':''}`}></span>
